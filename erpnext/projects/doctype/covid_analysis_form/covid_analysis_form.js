...
frappe.ui.form.on('COVID Analysis Form', {
	onload: function(frm) {
	...
	after_save:function(frm) {
		...
		frappe.call({
			method: "erpnext.projects.doctype.covid_analysis_form.covid_analysis_form.qrcode_gen",
			args: {
		...
		if(frm.doc.cus_info != null) {
			frappe.call({
				method: "erpnext.projects.doctype.covid_analysis_form.covid_analysis_form.set_rcv_flag",
				args: {
	...
	collect_sample: function(frm) {
		frappe.call({
			method: "erpnext.projects.doctype.covid_analysis_form.covid_analysis_form.update_collect_sample_flag",
			args: {
	...
	set_result: function(frm) {
		...
		frappe.call({
			method: "erpnext.projects.doctype.covid_analysis_form.covid_analysis_form.set_result_data",
			args: {
				company:frm.doc.company,
				custmer_name:frm.doc.custmer_name,
	...
	send_sms_message:function(frm) {
		...
		msgg += "http://94.142.51.110:1952/resultview?name="+ frm.doc.name +"&password="+ frm.doc.customer_password +  "\n";
		msgg += ")" +  "\n";
	...
	party_list: function(frm) {
		...
		if(frm.doc.party_list != null) {
			frappe.call({
				method: "erpnext.projects.doctype.covid_analysis_form.covid_analysis_form.get_party_list_data",
				args: {
	...
	medical_direction: function(frm) {
		if(frm.doc.medical_direction != null) {
			frappe.call({
				method: "erpnext.projects.doctype.covid_analysis_form.covid_analysis_form.get_medical_direction_data",
				args: {
					medical_direction:frm.doc.medical_direction
				},
				callback: function(r, rt) {
	...
	cus_info: function(frm) {
		if(frm.doc.cus_info != null) {
			frappe.call({
				method: "erpnext.projects.doctype.covid_analysis_form.covid_analysis_form.get_customer",
				args: {
					cus_info:frm.doc.cus_info
				},
				callback: function(r, rt) {
