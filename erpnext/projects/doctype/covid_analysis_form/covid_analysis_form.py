...
class COVIDAnalysisForm(Document):
	def __init__(self, *args, **kwargs):
		super(COVIDAnalysisForm, self).__init__(*args, **kwargs)
	...
	#		#self.barcode = get_barcode(self.company,self.medical_direction,self.custmer_name,self.date);
	#		frappe.db.set_value("COVID Analysis Form", {"company": self.company,"custmer_name": self.custmer_name}, "barcode", self.barcode)
	#		frappe.db.set_value("COVID Analysis Form", {"company": self.company,"custmer_name": self.custmer_name}, "daily_serial", self.daily_serial)
...
def qrcode_gen(customer_password,docname):
	...
	frappe.db.set_value("COVID Analysis Form", {"name": docname}, "qrcode_path", qrpath_db)
...
def update_collect_sample_flag(company,medical_direction,custmer_name,collection_date,collection_time,collection_users):
	...
	res = 1
	frappe.db.set_value("COVID Analysis Form", {"company": company,"custmer_name": custmer_name}, "sample_collect_flag", "1")
	#frappe.db.set_value("COVID Analysis Form", {"company": company,"custmer_name": custmer_name}, "barcode", barcode)
	frappe.db.set_value("COVID Analysis Form", {"company": company,"custmer_name": custmer_name}, "collection_date", collection_date)
	frappe.db.set_value("COVID Analysis Form", {"company": company,"custmer_name": custmer_name}, "collection_time", collection_time)
	frappe.db.set_value("COVID Analysis Form", {"company": company,"custmer_name": custmer_name}, "collection_users", collection_users)
...
def set_result_data(company,custmer_name,result_date,result_time,result_user):
	...
	frappe.db.set_value("COVID Analysis Form", {"company": company,"custmer_name": custmer_name}, "result_flage", "1")
	frappe.db.set_value("COVID Analysis Form", {"company": company,"custmer_name": custmer_name}, "result_date", result_date)
	frappe.db.set_value("COVID Analysis Form", {"company": company,"custmer_name": custmer_name}, "result_time", result_time)
	frappe.db.set_value("COVID Analysis Form", {"company": company,"custmer_name": custmer_name}, "result_user", result_user)
	frappe.db.set_value("COVID Analysis Form", {"company": company,"custmer_name": custmer_name}, "analysis_result", "Not Detected (Negative)")
	frappe.db.set_value("COVID Analysis Form", {"company": company,"custmer_name": custmer_name}, "analysis_result_ref_range", "Not Detected (Negative)")
