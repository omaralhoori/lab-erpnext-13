...
const setup_input_listeners = (frm) => {
	...
	$('.test-selected-btn').click(function() {
		...
		frappe.call({
			method: "erpnext.healthcare.doctype.clinical_testing.clinical_testing.apply_test_button_action",
			args: {
				action: button,
				tests: searchIDs,
...
frappe.ui.form.on('Clinical Testing', {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()){
				...
				frm.add_custom_button(__('Print'), function(){
					...
					frappe.call({
						...
						callback: function(res){
							if (res.message){
								let url = `/api/method/erpnext.healthcare.doctype.clinical_testing.clinical_testing.clnc_test_result?lab_test=${frm.doc.name}`
			...
			frappe.call({
				...
				// }
				var tests_html = format_tests_html(tests)
				$(frm.fields_dict.lab_test_html.wrapper).html(tests_html )
...
frappe.ui.form.on('Clinical Testing', 'patient', function (frm) {
	if (frm.doc.patient) {
		frappe.call({
