...
frappe.ui.form.on('Cheque Issued Entry', {
	onload: function(frm) {
		...
		frm.set_query("chq_info_name","cheque_transaction",  function(doc, cdt, cdn) {
			...
			return {
				query: 'erpnext.accounts.doctype.cheque_issued_entry.cheque_issued_entry.get_chq_list',
				filters: {
	...
	bank_account:function(frm){
		frappe.db.get_value("Bank Account", {"name":frm.doc.bank_account}, "bank").then((res) => {
			...
			frappe.db.get_value("Bank Account",  {"bank":res.message.bank,"account_type":"Cheque Issue Account"}, "account").then((r) => {
				frm.set_value('account_no', r.message.account)
			})
			frappe.db.get_value("Bank Account",  {"bank":res.message.bank,"account_type":"Deferred Cheque Account"}, "account").then((r) => {
				frm.set_value('deferred_account', r.message.account)
			})
		})
	},
	...
	entry_type: function(frm){
		...
		frm.set_value('deferred_account', '')
		...
		frm.set_query("bank_account", function() {
			return {
				filters: {
					...
					'account_type':["in",["Cheque Issue Account"]] 
					//'account_type':["in",["Cheque Collection Account","Current Account","Cheque Return Account"]] 
				}
			}
		});

		frm.set_query("deferred_account", function() {
			return {
				filters: {
					'is_company_account': 1,
					'account_type':["in",["Deferred Cheque Account"]] 
					//'account_type':["in",["Cheque Collection Account","Current Account","Cheque Return Account"]] 
				}
			}
		});
...
frappe.ui.form.on("Cheque Issued Transaction", {
	...
	chq_info_name: function(frm, cdt, cdn) {
		...
		if(row.chq_info_name != null) {
			frappe.call({
				method: 'erpnext.accounts.doctype.cheque_issued_entry.cheque_issued_entry.get_chq_info',
				args: {
